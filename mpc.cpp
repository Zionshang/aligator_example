...
int main()
{
    ...
    constexpr VerboseLevel verbosity = VerboseLevel::QUIET;
    ...
    for (auto t = 0.; t < 5.; t += dt)
    {
        ...
        ddp.workspace_.problem_data.term_cost_data = problem.term_cost_->createData();
        problem.setInitState(x);

        bool converged = ddp.run(problem);
        const auto [expected, support] = height(std::max(0., t - (nsteps - 1) * dt));
        const auto &actual = data.oMf[model.getFrameId("link")];

        const auto tf = std::chrono::steady_clock::now();
        const auto time1 = std::chrono::duration<double, std::milli>(t1 - t0);
        const auto time2 = std::chrono::duration<double, std::milli>(tf - t1);
        fmt::print("create model time: {} ms\n", time1.count());
